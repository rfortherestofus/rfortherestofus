...
theme_rru <- function(show_grid_lines = TRUE,
  ...
  if (void == TRUE) {
    ...
    rru_theme <- rru_theme +
      ...
      ggplot2::theme(plot.margin = ggplot2::margin(5.5, 5.5, 5.5, 5.5, unit = "points"))
  }
  ...
  rru_theme <- rru_theme +
    ggplot2::theme(text = ggplot2::element_text(color =  rru_colors("Light Text"), size = 14),
                   ...
                   legend.title = ggtext::element_markdown(color =  rru_colors("Dark Text"),
                                                           family = base_family, size = 14),
                   plot.subtitle = ggtext::element_markdown(family = base_family,
                                                            size = 16,
                                                            lineheight = 1.3))
